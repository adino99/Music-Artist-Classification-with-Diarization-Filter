...
def train_model(nb_classes=20,
    ...
    weights = os.path.join(save_weights_folder, str(nb_classes) +
                           '_' + str(slice_length) + '_' + str(random_states) + '.keras')
    ...
    # build the model
    model = models.CRNN2DVGG(X_train.shape, nb_classes=Y_train.shape[1])
    ...
    # Train the model
    if train:
        print("Input Data Shape", X_train.shape, "with random_states", random_states)

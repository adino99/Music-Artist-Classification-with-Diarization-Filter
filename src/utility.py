...
# import matplotlib.cm as cm  # Ensure cm is imported

# cmap = cm.get_cmap('Blues')  # Use cm.get_cmap

# # Example usage of cmap in a plot
...
# cmap = plt.get_cmap('Blues')  # Use plt.get_cmap
...
pipeline = Pipeline.from_pretrained(
  ...
  use_auth_token="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")
...
# +
def get_offset_duration(file_audio):
    diarization = pipeline(file_audio)
    # print the result
    offset = -1
    last = -1
    for turn, _, speaker in diarization.itertracks(yield_label=True):
        ...
        if last < turn.end:
            last = turn.end
    duration = last-offset
    print(f"File {file_audio} with singer offset={offset:.1f}s last={last:.1f}s duration={duration}s")
    return offset, duration
    
def create_dataset_diarization(artist_folder='artists', save_folder='song_data',
                   sr=16000, n_mels=128,
    ...
    for artist in artists:
        ...
        for album in artist_albums:
            ...
            for song in album_songs:
                ...
                offset, duration = get_offset_duration(song_path)
                y, sr = librosa.load(song_path, sr=sr, offset=offset, duration=duration)
...
def load_dataset(song_folder_name='song_data',
                 artist_folder='/workspace/artist20/mp3s-32k/',
...
def plot_confusion_matrix_OLD(cm, classes,
                          normalize=False,
...
def predict_artist(model, X, Y, S,
    ...
    for song in songs:
        ...
        prediction_mode = stats.mode(prediction_summary_trim)
        if isinstance(prediction_mode.mode, np.ndarray) and prediction_mode.mode.size > 0:
            prediction = prediction_mode.mode[0]
        else:
            prediction = prediction_summary_trim[0]

        # Get the actual class
        Y_song_class = np.argmax(Y_song, axis=1)
        actual_mode = stats.mode(Y_song_class)
        if isinstance(actual_mode.mode, np.ndarray) and actual_mode.mode.size > 0:
            actual = actual_mode.mode[0]
        else:
            actual = Y_song_class[0]
...
def predict_artist_OLD(model, X, Y, S,
                   le, class_names,
                   slices=None, verbose=False,
                   ml_mode=False):
    """
...
#     if create_data:
#         create_dataset(artist_folder='/workspace/artist20/mp3s-32k', save_folder='song_data',
#                        sr=16000, n_mels=128, n_fft=2048,
#                        hop_length=512)

    if create_data:
        # create_dataset(artist_folder='artist20/mp3s-32k', save_folder='song_data',
        #                sr=16000, n_mels=128, n_fft=2048,
        #                hop_length=512)
        create_dataset_diarization(artist_folder='/workspace/artist20/mp3s-32k', save_folder='song_data',
                       sr=16000, n_mels=128, n_fft=2048,
                       hop_length=512)
    ...
    if create_visuals:
        ...
        visualize_spectrogram(
            '/workspace/artist20/mp3s-32k/u2/The_Joshua_Tree/' +
            '02-I_Still_Haven_t_Found_What_I_m_Looking_For.mp3',
            offset=60, duration=29.12)

        # Create spectrogram subplots
        create_spectrogram_plots(artist_folder='/workspace/artist20/mp3s-32k', sr=16000, n_mels=128,
                                 n_fft=2048, hop_length=512)
        if save_visuals:
